...
SELECT
  ...
  COALESCE(u.handle, sm."memberHandle") AS "handle",
  e.address AS "email",
  ...
  CASE
    WHEN sm.is_marathon_match THEN mm_score."submissionScore"
    ELSE NULL
...
LEFT JOIN LATERAL (
  ...
  WHERE s."challengeId" = sm."challengeId"
    AND s."memberId" = sm."memberId"
) AS mm_score ON true
ORDER BY
