...
eligible_tasks AS (
  ...
  WHERE c.status = 'COMPLETED'
    AND COALESCE(c."taskIsTask", false) = true
    AND (
      proj.name IS NULL
      OR (
...
SELECT
  ...
  COALESCE(aggregated.task_count, 0)::bigint AS "cost_transaction.count_challenges"
FROM aggregated;
